...
function makeCalendar(dateStart, dateLength) {
	...
	for (var j = 0; j <= dateLength; j++) {
		...
		if ( j === 0 ) {
			...
			// Fix empty spaces array iteration length
			var loopLen = tempDay - 1;

			// Add all invalid day spaces before start date
			addEmptyDaySpaces(tempYear, tempMonth, loopLen, false);
...
function addEmptyDaySpaces(year, month, length, empty) {
